...
export default async function GroomersPage() {
  ...
  const { data: groomersData, error } = await supabase
    .from('groomers')
    .select('id, name, email, phone, role, color, avatar_url, is_active')
    .eq('is_active', true)
    .order('name', { ascending: true })

  const groomers: Groomer[] = groomersData || []
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          ...
          <p className="text-2xl font-bold text-gray-900">{groomers.length}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-sm text-gray-500">Active Groomers</p>
          <p className="text-2xl font-bold text-gray-900">
            {groomers.filter(m => m.role.toLowerCase().includes('groomer')).length}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-sm text-gray-500">Owners/Admins</p>
          <p className="text-2xl font-bold text-gray-900">
            {groomers.filter(m => m.role.toLowerCase() === 'owner' || m.role.toLowerCase() === 'admin').length}
          </p>
        </div>
      </div>
