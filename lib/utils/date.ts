...
/**
 * Get a local YYYY-MM-DD key from a date/time value.
 * Useful for grouping/filtering by local calendar day.
 */
...
/**
 * Build an ISO string (UTC) from a local date and time input.
 * Expects date in YYYY-MM-DD and time in HH:MM (24h).
 */
export function buildLocalDateTimeISO(
  dateStr: string,
  timeStr: string
): string | null {
